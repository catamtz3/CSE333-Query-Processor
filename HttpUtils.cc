...
bool IsPathSafe(const string& root_dir, const string& test_file) {
  ...
  // STEP 1
  char root_buf[PATH_MAX], test_buf[PATH_MAX];

  if (!realpath(root_dir.c_str(), root_buf)) {
    return false;
  }
  if (!realpath(test_file.c_str(), test_buf)) {
    return false;
  }

  if (strlen(test_buf) <= strlen(root_buf)) {
    return false;
  }
  if (test_buf[strlen(root_buf)] != '/') {
    return false;
  }
